    ...
    <div class="panel">
        ...
        <el-collapse v-model="activeNames">
            <el-collapse-item name="1">
                <template slot="title">
                    <i class="collapse-icon el-icon-s-order"></i><span class="collapse-title">当前模拟任务</span>
                </template>
                <div class="task-info">
                    <div>
                    ...
                    <div>
                        ...
                        <div class="info-content"
                            v-show="taskData.type == 'infection' && taskData.infectionModel == 'gamma'">
                            <el-row>
                                <el-col :span="8" class="line">
                                    <ul type="circle" class="label">
                                        <li>
            ...
            <el-collapse-item name="2">
                <template slot="title">
                    <i class="collapse-icon el-icon-s-order"></i><span class="collapse-title">历史模拟任务列表</span>
                </template>
        ...
        <el-drawer :title="drawType == 'flood' ? '暴雨灾害模拟结果' : '疫情模拟结果'" :visible.sync="drawer" :modal="false" size="78%">
            <flodsimulation :type="drawType" :status="status" :metric="metric" :idSuffix="'-simu'"
                v-show="drawType == 'flood'" />
            <infectionSimulation :type="drawType" :infectionModel="infectionModel" :status="status" :idSuffix="'-simu'"
...
import flodsimulation from "@/views/simulator/components/disasterSimulator-ol.vue";
import infectionSimulation from "@/views/simulator/components/infectionSimulation-ol.vue";
    ...
    methods: {
        ...
        introduction() {
            introJs().setOptions({
                ...
                steps: [{
                    intro: '欢迎来到<strong>风险预测</strong>界面！'
                },
                {
                    intro: '查看<strong>当前模拟任务</strong>信息',
                    element: document.querySelector('.task-info'),
                },
                {
                    intro: '查看<strong>模拟结果</strong>',
                    element: document.querySelector('.task-control button:nth-of-type(1)'),
                },
                {
                    intro: '完成<strong>风险预测</strong>',
                    element: document.querySelector('.dialog4 .dialog-footer button:nth-of-type(2)'),
                },
                ]
            }).start();
        },
        ...
        getCurrentTask() {
            ...
            this.$emit("changeCurrentTask", this.currentTask);
        },
        ...
        submitNewTask() {
            let params = Object.assign({}, this.taskData.params);
            params["Whether drainage facilities are used"] = "true";
            ...
            let url = serverInfo.baseURL + "/simulation/doubleSimulator/" + this.currentTask.id;
            let headers = {
